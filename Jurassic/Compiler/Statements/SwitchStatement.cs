...
namespace Jurassic.Compiler
{
    ...
    internal class SwitchStatement : Statement
    {
        ...
        public override void GenerateCode(ILGenerator generator, OptimizationInfo optimizationInfo)
        {
            ...
            StatementLocals statementLocals = new StatementLocals() { NonDefaultBreakStatementBehavior = true };
            ...
            ILLabel[] jumpTargets = new ILLabel[this.CaseClauses.Count];
            int defaultIndex = -1;
            ILLabel endOfSwitch = generator.CreateLabel();

            // Generate code for the switch value.
            ILLabel startOfSwitch = generator.CreateLabel();
            ...
            ILLocalVariable switchValue = generator.CreateTemporaryVariable(typeof(object));
            generator.StoreVariable(switchValue);

            for (int i = 0; i < this.CaseClauses.Count; i ++)
            {
                SwitchCase caseClause = this.CaseClauses[i];
            ...
            for (int i = 0; i < this.CaseClauses.Count; i++)
            {
                ...
                foreach (Statement statement in this.CaseClauses[i].BodyStatements) {
        ...
        public override IEnumerable<AstNode> ChildNodes
        {
            get
            {
                ...
                foreach (SwitchCase clause in this.CaseClauses)
                {
                    if (clause.Value != null) {
                        yield return clause.Value;
                    }

                    foreach (Statement statement in clause.BodyStatements) {
        ...
        public override string ToString(int indentLevel)
        {
            ...
            result.AppendLine("{");
            indentLevel++;
            foreach (SwitchCase caseClause in this.CaseClauses)
            {
                result.Append(new string('\t', indentLevel));
                if (caseClause.Value != null)
                {
                ...
                foreach (Statement statement in caseClause.BodyStatements) {
