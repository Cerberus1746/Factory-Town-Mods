...
namespace Jurassic.Compiler
{
    ...
    internal abstract class LoopStatement : Statement
    {
        ...
        public override void GenerateCode(ILGenerator generator, OptimizationInfo optimizationInfo)
        {
            ...
            StatementLocals statementLocals = new StatementLocals() { NonDefaultBreakStatementBehavior = true, NonDefaultSourceSpanBehavior = true };
            ...
            // Set up some labels.
            ILLabel continueTarget1 = generator.CreateLabel();
            ILLabel continueTarget2 = generator.CreateLabel();
            ILLabel breakTarget1 = generator.CreateLabel();
            ILLabel breakTarget2 = generator.CreateLabel();
            ...
            bool previousInsideTryCatchOrFinally = optimizationInfo.InsideTryCatchOrFinally;
            if (optimizationInfo.OptimizeInferredTypes == true)
            {
                ...
                Dictionary<Scope.DeclaredVariable, InferredTypeInfo> typedVariables = this.FindTypedVariables();
                foreach (KeyValuePair<Scope.DeclaredVariable, InferredTypeInfo> variableAndType in typedVariables)
                {
                    Scope.DeclaredVariable variable = variableAndType.Key;
                    InferredTypeInfo variableInfo = variableAndType.Value;
                    if (variableInfo.Conditional == false && variableInfo.Type != variable.Type)
                    {
                        // Save the previous type so we can restore it later.
                        PrimitiveType previousType = variable.Type;
                        ...
                        NameExpression nameExpression = new NameExpression(variable.Scope, variable.Name);
            ...
            ILLabel startOfLoop = generator.DefineLabelPosition();
            ...
            if (previousVariableTypes != null && previousVariableTypes.Count > 0)
            {
                ...
                foreach (KeyValuePair<Scope.DeclaredVariable, RevertInfo> previousVariableAndType in previousVariableTypes)
                {
                    Scope.DeclaredVariable variable = previousVariableAndType.Key;
                    RevertInfo variableRevertInfo = previousVariableAndType.Value;
        ...
        {
            Dictionary<Scope.DeclaredVariable, InferredTypeInfo> result = new Dictionary<Scope.DeclaredVariable, InferredTypeInfo>();
            ...
            if (everythingIsOkay == true)
            {
                ...
                Scope.DeclaredVariable variable = loopVariableScope.GetDeclaredVariable(loopVariableName);
                if (variable != null) {
        ...
        private static void FindTypedVariables(AstNode root, Dictionary<Scope.DeclaredVariable, InferredTypeInfo> variableTypes, bool conditional, ref bool continueEncountered)
        {
            ...
            {
                // Found an assignment.
                AssignmentExpression assignment = (AssignmentExpression)root;
                if (assignment.Target is NameExpression)
                {
                    // Found an assignment to a variable.
                    NameExpression name = (NameExpression)assignment.Target;
                    if (name.Scope is DeclarativeScope)
                    {
                        Scope.DeclaredVariable variable = name.Scope.GetDeclaredVariable(name.Name);
                        if (variable != null)
                        {
            ...
            foreach (AstNode node in root.ChildNodes) {
