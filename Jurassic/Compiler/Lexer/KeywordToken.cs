...
namespace Jurassic.Compiler
{
    ...
    internal class KeywordToken : Token
    {
        ...
        {
            Dictionary<string, Token> result = new Dictionary<string, Token>(keywords.Length + additionalKeywords.Length);
            foreach (Token token in keywords) {
                result.Add(token.Text, token);
            }

            foreach (Token token in additionalKeywords) {
                result.Add(token.Text, token);
            }
