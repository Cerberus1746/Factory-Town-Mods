...
namespace Jurassic.Compiler
{
    ...
    internal abstract class Binder
    {
        ...
        {
            BinderDelegate binderDelegate = this.CreateDelegate(arguments.Length);
        ...
        public BinderDelegate CreateDelegate(int argumentCount)
        {
            ...
            if (argumentCount > MaximumCachedParameterCount) {
                return this.CreateDelegateCore(argumentCount);
            }
            ...
            BinderDelegate binderDelegate = this.delegateCache[argumentCount];
            if (binderDelegate == null)
            {
                // Create a binding method.
                binderDelegate = this.CreateDelegateCore(argumentCount);
        ...
        private BinderDelegate CreateDelegateCore(int argumentCount)
        {
            ...
            if (ScriptEngine.LowPrivilegeEnvironment == false)
            {
                ...
                generator = Environment.Version.Major >= 4 && !ScriptEngine.IsMonoRuntime
                    ? new DynamicILGenerator(dm)
                    : (ILGenerator) new ReflectionEmitILGenerator(dm.GetILGenerator());
            }
            else
            {
#endif
            ...
            this.GenerateStub(generator, argumentCount);
