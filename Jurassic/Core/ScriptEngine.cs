...
namespace Jurassic
{
...
#endif
    {
        ...
        private readonly HiddenClassSchema emptySchema;
        ...
        public ScriptEngine()
        {
            ...
            ObjectInstance baseObject = ObjectInstance.CreateRootObject(this);

            // Create the global object.
            this.globalObject = new GlobalObject(baseObject);

            // Create the function object that second to last in the prototype chain.
            UserDefinedFunction baseFunction = UserDefinedFunction.CreateEmptyFunction(baseObject);
        ...
        internal static bool LowPrivilegeEnvironment
        {
            get
            {
                lock (lowPrivilegeEnvironmentLock)
                {
                    ...
                    {
                        System.Security.Permissions.SecurityPermission permission = new System.Security.Permissions.SecurityPermission(
        ...
        {
            Compiler.GlobalMethodGenerator methodGen = new Jurassic.Compiler.GlobalMethodGenerator(
                this,                               // The script engine
                source,                             // The source code.
                this.CreateOptions());                   // The compiler options.
        ...
        public object Evaluate(string code) => this.Evaluate(new StringScriptSource(code));
        ...
        /// <returns> The result of executing the source code. </returns>
        /// <exception cref="ArgumentNullException"> <paramref name="code"/> is a <c>null</c> reference. </exception>
        public T Evaluate<T>(string code) => TypeConverter.ConvertTo<T>(this, this.Evaluate(code));
        ...
        {
            Compiler.EvalMethodGenerator methodGen = new Jurassic.Compiler.EvalMethodGenerator(
                this,                               // The script engine
            ...
            object result = methodGen.Execute();
        ...
        /// <returns> The result of executing the source code. </returns>
        /// <exception cref="ArgumentNullException"> <paramref name="code"/> is a <c>null</c> reference. </exception>
        public T Evaluate<T>(ScriptSource source) => TypeConverter.ConvertTo<T>(this, this.Evaluate(source));
        ...
        public void Execute(string code) => this.Execute(new StringScriptSource(code));
        ...
        /// UNC path. </param>
        /// <exception cref="ArgumentNullException"> <paramref name="path"/> is a <c>null</c> reference. </exception>
        public void ExecuteFile(string path) => this.ExecuteFile(path, null);
        ...
        /// mark (BOM). </param>
        /// <exception cref="ArgumentNullException"> <paramref name="path"/> is a <c>null</c> reference. </exception>
        public void ExecuteFile(string path, System.Text.Encoding encoding) => this.Execute(new FileScriptSource(path, encoding));
        ...
        {
            // Compile the script.
            CompiledScript compiledScript = this.Compile(source);
        ...
        private Compiler.CompilerOptions CreateOptions() => new Compiler.CompilerOptions() { ForceStrictMode = this.ForceStrictMode, EnableDebugging = this.EnableDebugging };
        ...
        public object CallGlobalFunction(string functionName, params object[] argumentValues)
        {
            ...
            object value = this.Global.GetPropertyValue(functionName);
        ...
        public T CallGlobalFunction<T>(string functionName, params object[] argumentValues)
        {
            ...
            return TypeConverter.ConvertTo<T>(this, this.CallGlobalFunction(functionName, argumentValues));
        }
        ...
        public void SetGlobalFunction(string functionName, Delegate functionDelegate)
        {
            ...
            this.SetGlobalValue(functionName, new ClrFunction(this.Function.InstancePrototype, functionDelegate, functionName));
        }
        ...
        internal Compiler.ObjectScope CreateGlobalScope() => Compiler.ObjectScope.CreateGlobalScope(this.Global);
        ...
        internal object Eval(string code, Compiler.Scope scope, object thisObject, bool strictMode)
        {
            ...
            Compiler.CompilerOptions options = new Compiler.CompilerOptions() { ForceStrictMode = strictMode };
            Compiler.EvalMethodGenerator evalGen = new Jurassic.Compiler.EvalMethodGenerator(
                this,                                                   // The script engine.
        ...
        internal string FormatStackTrace(string errorName, string message, string path, string function, int line)
        {
            ...
            if (path != null || function != null || line != 0) {
                this.AppendStackFrame(result, path, function, line);
            }

            foreach (StackFrame frame in this.stackFrames) {
                this.AppendStackFrame(result, frame.Path, frame.Function, frame.Line);
            }
        ...
        internal void PushStackFrame(string path, string function, int line) => this.stackFrames.Push(new StackFrame() { Path = path, Function = function, Line = line });

        /// <summary>
        /// Pops a frame from the javascript stack.
        /// </summary>
        internal void PopStackFrame() => this.stackFrames.Pop();
