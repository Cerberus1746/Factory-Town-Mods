...
namespace Jurassic
{
    ...
    internal static class NumberParser
    {
        ...
        {
            StringReader reader = new System.IO.StringReader(input);
        ...
        private static double RefineEstimate(double initialEstimate, int base10Exponent, BigInteger desiredValue)
        {
            ...
            while (precision <= 160)
            {
                ...
                BigInteger actual1 = ScaleToInteger(result, multiplier, precision);
                BigInteger actual2 = ScaleToInteger(result2, multiplier, precision);

                // Calculate the differences between the candidate values and the desired value.
                BigInteger baseline = BigInteger.LeftShift(desiredValue, precision);
                BigInteger diff1 = BigInteger.Subtract(actual1, baseline);
                BigInteger diff2 = BigInteger.Subtract(actual2, baseline);
