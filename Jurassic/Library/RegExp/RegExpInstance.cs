...
namespace Jurassic.Library
{
    ...
    public class RegExpInstance : ObjectInstance
    {
            ...
            : base(prototype)
        {
            ...
            try
            {
                this.value = new Regex(pattern, this.ParseFlags(flags));
            }
        ...
        public string Flags
        {
            get
            {
                System.Text.StringBuilder result = new System.Text.StringBuilder(3);
                if (this.Global) {
                    result.Append("g");
                }
        ...
        {
#if !SILVERLIGHT
            this.value = new Regex(pattern, this.ParseFlags(flags) | RegexOptions.Compiled);
#endif
        ...
        public bool Test(string input)
        {
            ...
            Match match = this.value.Match(input, this.CalculateStartPosition(input));
        ...
        public object Exec(string input)
        {
            ...
            ArrayInstance result = this.Engine.Array.New(array);
        ...
        public object Match(string input)
        {
            ...
            MatchCollection matches = this.value.Matches(input);
            if (matches.Count == 0) {
                return Null.Value;
            }
        ...
        public string Replace(string input, string replaceText)
        {
            ...
            string result = this.value.Replace(input, match =>
            {
                ...
                System.Text.StringBuilder replacementBuilder = new System.Text.StringBuilder();
        ...
        /// <returns> A copy of the given string with text replaced using a regular expression. </returns>
        public string Replace(string input, FunctionInstance replaceFunction) => this.value.Replace(input, match => {
            // Set the deprecated RegExp properties.
            this.Engine.RegExp.SetDeprecatedProperties(input, match);

            object[] parameters = new object[match.Groups.Count + 2];
            for (int i = 0; i < match.Groups.Count; i++) {
                parameters[i] = match.Groups[i].Success == false ? Undefined.Value : (object) match.Groups[i].Value;
            }
        ...
        {
            // Perform the regular expression matching.
            Match match = this.value.Match(input);
        ...
        public new string ToString() => string.Format("/{0}/{1}", this.Source, this.Flags);
