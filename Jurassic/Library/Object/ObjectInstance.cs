...
namespace Jurassic.Library
{
...
#endif
    {
        ...
        internal static ObjectInstance CreateRootObject(ScriptEngine engine) => new ObjectInstance(engine, null);
        ...
        internal static ObjectInstance CreateRawObject(ObjectInstance prototype) => new ObjectInstance(prototype);
        ...
        void System.Runtime.Serialization.IDeserializationCallback.OnDeserialization(object sender) => this.OnDeserializationCallback();
        ...
        public object this[string propertyName]
        {
            ...
            set { this.SetPropertyValue(propertyName, value, false); }
        }
        ...
        {
            get { return this.GetPropertyValue(index); }
            set { this.SetPropertyValue(index, value, false); }
        }
        ...
        public object this[int index]
        {
            get
            {
                ...
                return this.GetPropertyValue((uint)index);
            }
            set
            {
                if (index < 0) {
                ...
                this.SetPropertyValue((uint)index, value, false);
            }
        }
        ...
        {
            SchemaProperty propertyInfo = this.schema.GetPropertyIndexAndAttributes(name);
            ...
            else
            {
                ...
                if (this.Prototype == null) {
                    return this.GetMissingPropertyValue(name);
                }
        ...
        public void InlineSetPropertyValue(string name, object value, bool throwOnError, out int cachedIndex, out object cacheKey)
        {
            ...
            if (propertyInfo.Exists == true)
            {
                ...
                if ((propertyInfo.Attributes & (PropertyAttributes.Writable | PropertyAttributes.IsAccessorProperty | PropertyAttributes.IsLengthProperty)) != PropertyAttributes.Writable)
                {
                    cachedIndex = -1;
                    cacheKey = null;
                    this.SetPropertyValue(name, value, throwOnError);
                }
                else
                {
                    // The property can be cached.
        ...
        public bool InlineSetPropertyValueIfExists(string name, object value, bool throwOnError, out int cachedIndex, out object cacheKey)
        {
            ...
            if (propertyInfo.Exists == true)
            {
                ...
                if ((propertyInfo.Attributes & (PropertyAttributes.Writable | PropertyAttributes.IsAccessorProperty | PropertyAttributes.IsLengthProperty)) != PropertyAttributes.Writable)
                {
                    cachedIndex = -1;
                    cacheKey = null;
                    return this.SetPropertyValueIfExists(name, value, throwOnError);
                }
                else
                {
                    // The property can be cached.
        ...
        public bool HasProperty(string propertyName) => this.GetPropertyValue(propertyName) != null;
        ...
        /// this object. </remarks>
        public object GetPropertyValue(uint index) => this.GetPropertyValue(index, this);
        ...
        private object GetPropertyValue(uint index, ObjectInstance thisValue)
        {
            ...
            if (property.Exists == true)
            {
                ...
                PropertyAccessorValue accessor = value as PropertyAccessorValue;
                if (accessor != null) {
        ...
        public object GetPropertyValue(string propertyName)
        {
            ...
            if (arrayIndex != uint.MaxValue) {
                return this.GetPropertyValue(arrayIndex);
            }

            // Otherwise, the property is a name.
            return this.GetNamedPropertyValue(propertyName, this);
        }
        ...
        private object GetNamedPropertyValue(string propertyName, ObjectInstance thisValue)
        {
            ...
            do
            {
                ...
                SchemaProperty property = prototypeObject.schema.GetPropertyIndexAndAttributes(propertyName);
                if (property.Exists == true)
                {
        ...
        {
            SchemaProperty property = this.schema.GetPropertyIndexAndAttributes(index.ToString());
            if (property.Exists == true) {
        ...
        public PropertyDescriptor GetOwnPropertyDescriptor(string propertyName)
        {
            ...
            uint arrayIndex = ArrayInstance.ParseArrayIndex(propertyName);
            if (arrayIndex != uint.MaxValue) {
                return this.GetOwnPropertyDescriptor(arrayIndex);
            }

            // Retrieve information about the property.
            SchemaProperty property = this.schema.GetPropertyIndexAndAttributes(propertyName);
            if (property.Exists == true)
            {
        ...
        public virtual void SetPropertyValue(uint index, object value, bool throwOnError)
        {
            ...
            bool exists = this.SetPropertyValueIfExists(indexStr, value, throwOnError);
            if (exists == false)
            {
                // The property doesn't exist - add it.
                this.AddProperty(indexStr, value, PropertyAttributes.FullAccess, throwOnError);
            }
        }
        ...
        public void SetPropertyValue(string propertyName, object value, bool throwOnError)
        {
            ...
            if (arrayIndex != uint.MaxValue)
            {
                this.SetPropertyValue(arrayIndex, value, throwOnError);
                return;
            }

            bool exists = this.SetPropertyValueIfExists(propertyName, value, throwOnError);
            if (exists == false)
            {
                // The property doesn't exist - add it.
                this.AddProperty(propertyName, value, PropertyAttributes.FullAccess, throwOnError);
            }
        }
        ...
        public virtual bool Delete(uint index, bool throwOnError)
        {
            ...
            SchemaProperty propertyInfo = this.schema.GetPropertyIndexAndAttributes(indexStr);
        ...
        public bool Delete(string propertyName, bool throwOnError)
        {
            ...
            uint arrayIndex = ArrayInstance.ParseArrayIndex(propertyName);
            if (arrayIndex != uint.MaxValue) {
                return this.Delete(arrayIndex, throwOnError);
            }

            // Retrieve the attributes for the property.
            SchemaProperty propertyInfo = this.schema.GetPropertyIndexAndAttributes(propertyName);
            if (propertyInfo.Exists == false) {
                return true;    // Property doesn't exist - delete succeeded!
            }
        ...
        public virtual bool DefineProperty(string propertyName, PropertyDescriptor descriptor, bool throwOnError)
        {
            ...
            SchemaProperty current = this.schema.GetPropertyIndexAndAttributes(propertyName);

            if (current.Exists == false)
            {
                // Create a new property.
                return this.AddProperty(propertyName, descriptor.Value, descriptor.Attributes, throwOnError);
            }
        ...
        internal void FastSetProperty(string propertyName, object value, PropertyAttributes attributes = PropertyAttributes.Sealed, bool overwriteAttributes = false)
        {
            ...
            if (index < 0)
            {
                ...
                this.AddProperty(propertyName, value, attributes, false);
                return;
            }
        ...
        protected internal virtual object GetPrimitiveValue(PrimitiveTypeHint typeHint)
        {
            if (typeHint == PrimitiveTypeHint.None || typeHint == PrimitiveTypeHint.Number)
            {
                ...
                if (this.TryCallMemberFunction(out valueOfResult, "valueOf") == true)
                {
                ...
                if (this.TryCallMemberFunction(out toStringResult, "toString") == true)
                {
                    // Return value must be primitive.
        ...
        {
            object function = this.GetPropertyValue(functionName);
            if (function == null) {
        ...
        public override string ToString()
        {
            try
            {
                ...
                if (this.HasProperty("toString") == true)
                {
                    // Call the toString() method.
                    object result = this.CallMemberFunction("toString");
        ...
        public static bool IsPrototypeOf(ScriptEngine engine, object thisObject, object obj)
        {
            ...
            ObjectInstance obj2 = obj as ObjectInstance;
            while (true)
            {
                obj2 = obj2.Prototype;
                if (obj2 == null) {
        ...
        public static bool PropertyIsEnumerable(ScriptEngine engine, object thisObject, string propertyName)
        {
            ...
            PropertyDescriptor property = TypeConverter.ToObject(engine, thisObject).GetOwnPropertyDescriptor(propertyName);
        ...
        public string ToLocaleString() => TypeConverter.ToString(this.CallMemberFunction("toString"));
        ...
        public ObjectInstance ValueOf() => this;
        ...
        /// </summary>
        internal protected void PopulateFunctions() => this.PopulateFunctions(null);
        ...
        internal protected void PopulateFunctions(Type type) => this.PopulateFunctions(type, BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly);
        ...
        internal protected void PopulateFunctions(Type type, BindingFlags bindingFlags)
        {
            ...
            Dictionary<string, MethodGroup> functions = new Dictionary<string, MethodGroup>(20);
            MethodInfo[] methods = type.GetMethods(bindingFlags);
            foreach (MethodInfo method in methods)
            {
                // Make sure the method has the [JSInternalFunction] attribute.
                JSFunctionAttribute attribute = (JSFunctionAttribute)Attribute.GetCustomAttribute(method, typeof(JSFunctionAttribute));
                if (attribute == null) {
                    continue;
                }

                // Determine the name of the method.
                string name = attribute.Name != null ? attribute.Name : method.Name;
            ...
            {
                JSPropertyAttribute attribute = Attribute.GetCustomAttribute(prop, typeof(JSPropertyAttribute), false) as JSPropertyAttribute;
                if (attribute == null) {
                    continue;
                }

                // The property name.
                string name = attribute.Name != null ? attribute.Name : prop.Name;
                ...
                if (prop.CanRead)
                {
                    ...
                    getter = new ClrFunction(this.engine.Function.InstancePrototype, new JSBinderMethod[] { new JSBinderMethod(getMethod) }, name, 0);
                }
                ...
                if (prop.CanWrite)
                {
                    MethodInfo setMethod = prop.GetSetMethod((bindingFlags & BindingFlags.NonPublic) != 0);
                    if (setMethod != null) {
                        setter = new ClrFunction(this.engine.Function.InstancePrototype, new JSBinderMethod[] { new JSBinderMethod(setMethod) }, name, 1);
                    }
                }
        ...
        /// </summary>
        internal protected void PopulateFields() => this.PopulateFields(null);
        ...
        internal protected void PopulateFields(Type type)
        {
            ...
            {
                JSFieldAttribute attribute = (JSFieldAttribute)Attribute.GetCustomAttribute(field, typeof(JSFieldAttribute));
                if (attribute == null) {
                    continue;
                }
