...
namespace Jurassic.Library
{
    ...
    public class ObjectConstructor : ClrFunction
    {
        ...
        public ObjectInstance Construct() => ObjectInstance.CreateRawObject(this.InstancePrototype);
        ...
        {
            ObjectInstance result = obj.Prototype;
            if (result == null) {
                return Null.Value;
            }
        ...
        {
            PropertyDescriptor descriptor = obj.GetOwnPropertyDescriptor(propertyName);
        ...
        {
            ArrayInstance result = obj.Engine.Array.New();
            foreach (PropertyNameAndValue property in ((ObjectInstance)obj).Properties) {
        ...
        public static ObjectInstance Create(ScriptEngine engine, object prototype, [DefaultParameterValue(null)] ObjectInstance properties = null)
        {
            ...
            ObjectInstance result = prototype == Null.Value ? ObjectInstance.CreateRootObject(engine) : ObjectInstance.CreateRawObject((ObjectInstance)prototype);
            if (properties != null) {
        ...
        {
            PropertyDescriptor defaults = obj.GetOwnPropertyDescriptor(propertyName);
            PropertyDescriptor descriptor = PropertyDescriptor.FromObject(attributes, defaults);
        ...
        {
            foreach (PropertyNameAndValue property in properties.Properties) {
                if (property.IsEnumerable == true)
                {
                    object descriptor = property.Value;
        ...
        {
            List<PropertyNameAndValue> properties = new List<PropertyNameAndValue>();
            foreach (PropertyNameAndValue property in obj.Properties) {
                properties.Add(property);
            }

            foreach (PropertyNameAndValue property in properties)
            {
        ...
        public static new bool IsExtensible([JSParameter(JSParameterFlags.DoNotConvert)] ObjectInstance obj) => obj.IsExtensible;
