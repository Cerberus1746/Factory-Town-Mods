...
namespace Jurassic.Library
{
    ...
    internal class HiddenClassSchema
    {
        ...
        public static HiddenClassSchema CreateEmptySchema() => new HiddenClassSchema(new Dictionary<string, SchemaProperty>(), 0);
        ...
        public int PropertyCount
        {
            get
            {
                if (this.properties == null) {
                    this.properties = this.CreatePropertiesDictionary();
                }
        ...
        public IEnumerable<PropertyNameAndValue> EnumeratePropertyNamesAndValues(object[] values)
        {
            ...
            foreach (KeyValuePair<string, SchemaProperty> pair in this.properties) {
        ...
        public int GetPropertyIndex(string name) => this.GetPropertyIndexAndAttributes(name).Index;
        ...
        public HiddenClassSchema AddProperty(string name, PropertyAttributes attributes = PropertyAttributes.FullAccess)
        {
            ...
            TransitionInfo transitionInfo = new TransitionInfo() { Name = name, Attributes = attributes };
            ...
            if (newSchema == null)
            {
                if (this.parent == null)
                {
                    ...
                    Dictionary<string, SchemaProperty> properties = new Dictionary<string, SchemaProperty>(this.properties);
        ...
        public HiddenClassSchema DeleteProperty(string name)
        {
            ...
            if (newSchema == null)
            {
                ...
                Dictionary<string, SchemaProperty> properties = this.properties == null ? this.CreatePropertiesDictionary() : new Dictionary<string, SchemaProperty>(this.properties);
        ...
        private Dictionary<string, SchemaProperty> CreatePropertiesDictionary()
        {
            ...
            Stack<KeyValuePair<string, SchemaProperty>> addTransitions = new Stack<KeyValuePair<string, SchemaProperty>>();
            HiddenClassSchema node = this;
            while (node != null)
            {
            ...
            Dictionary<string, SchemaProperty> result = new Dictionary<string, SchemaProperty>(node.properties);
            while (addTransitions.Count > 0)
            {
                KeyValuePair<string, SchemaProperty> keyValuePair = addTransitions.Pop();
