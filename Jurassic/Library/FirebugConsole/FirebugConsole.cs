...
namespace Jurassic.Library
{
    ...
    public class FirebugConsole : ObjectInstance
    {
        ...
        public void Log(params object[] items) => this.Log(FirebugConsoleMessageStyle.Regular, items);
        ...
        public void Debug(params object[] items) => this.Log(FirebugConsoleMessageStyle.Regular, items);
        ...
        public void Info(params object[] items) => this.Log(FirebugConsoleMessageStyle.Information, items);
        ...
        public void Warn(params object[] items) => this.Log(FirebugConsoleMessageStyle.Warning, items);
        ...
        public void Error(params object[] items) => this.Log(FirebugConsoleMessageStyle.Error, items);
        ...
        public void Clear(params object[] items) => this.output.Clear();
        ...
        public void Group(params object[] items) => this.output.StartGroup(Format(items), false);
        ...
        public void GroupCollapsed(params object[] items) => this.output.StartGroup(Format(items), true);

        /// <summary>
        /// Closes the most recently opened block created by a call to console.group().
        /// </summary>
        [JSInternalFunction(Name = "groupEnd")]
        public void GroupEnd() => this.output.EndGroup();
        ...
        public void TimeEnd([DefaultParameterValue("")] string name = "")
        {
            ...
            Stopwatch stopwatch = this.timers[name];
            if (string.IsNullOrEmpty(name)) {
                this.Log(FirebugConsoleMessageStyle.Regular, string.Format("{0}ms", stopwatch.ElapsedMilliseconds));
            } else {
                this.Log(FirebugConsoleMessageStyle.Regular, string.Format("{0}: {1}ms", name, stopwatch.ElapsedMilliseconds));
            }
        ...
        /// color). </param>
        /// <param name="items"> The items to format. </param>
        private void Log(FirebugConsoleMessageStyle style, params object[] items) => this.output.Log(style, FormatObjects(items));
            ...
            if (items[0] is string)
            {
                ...
                while (items.Length > itemsConsumed)
                {
                    ...
                    switch (patternType)
                    {
                        ...
                        case 'd':
                        case 'i':
                            double number = TypeConverter.ToNumber(items[itemsConsumed++]);
