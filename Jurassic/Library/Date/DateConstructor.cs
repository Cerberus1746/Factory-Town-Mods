...
namespace Jurassic.Library
{
    ...
    public class DateConstructor : ClrFunction
    {
        ...
        /// </summary>
        [JSCallFunction]
        public string Call() => new DateInstance(this.InstancePrototype).ToStringJS();

        /// <summary>
        /// Creates a new Date object and sets its value to the current time.
        /// </summary>
        public DateInstance Construct() => new DateInstance(this.InstancePrototype);
        ...
        public DateInstance Construct(double milliseconds) => new DateInstance(this.InstancePrototype, milliseconds);
        ...
        public DateInstance Construct(string dateStr) => new DateInstance(this.InstancePrototype, dateStr);
        ...
        public DateInstance Construct(int year, int month, [DefaultParameterValue(1)] int day = 1, [DefaultParameterValue(0)] int hour = 0,
            [DefaultParameterValue(0)] int minute = 0, [DefaultParameterValue(0)] int second = 0, [DefaultParameterValue(0)] int millisecond = 0) => new DateInstance(this.InstancePrototype, year, month, day, hour, minute, second, millisecond);
        ...
        public DateInstance Construct(params object[] components)
        {
            ...
            if (components.Length == 1)
            {
                if (TypeUtilities.IsString(components[0]) == true) {
                    return this.Construct(TypeConverter.ToString(components[0]));
                }

                return this.Construct(TypeConverter.ToNumber(components[0]));
            }
            ...
            for (int i = 0; i < Math.Min(components.Length, 7); i++)
            {
                ...
                if (component < int.MinValue || component > int.MaxValue || double.IsNaN(component) == true) {
                    return this.Construct(double.NaN);
                }
            }
        ...
        public static double Now() => DateInstance.Now();
        ...
        public static double UTC(int year, int month, [DefaultParameterValue(1)] int day = 1, [DefaultParameterValue(0)] int hour = 0,
            [DefaultParameterValue(0)] int minute = 0, [DefaultParameterValue(0)] int second = 0, [DefaultParameterValue(0)] int millisecond = 0) => DateInstance.UTC(year, month, day, hour, minute, second, millisecond);
        ...
        public static double Parse(string dateStr) => DateInstance.Parse(dateStr);
    }
}
