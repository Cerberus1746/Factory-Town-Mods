    ...
    public static class DateParser
    {
        ...
        internal static DateTime Parse(string input)
        {
            ...
            if (match.Success == true)
            {
                ...
                DateTime result = new DateTime(year, month, day, hour, minute, second, millisecond, DateTimeKind.Utc);
        ...
        private static DateTime ParseUnstructured(string input)
        {
            ...
            {
                HashSet<string> temp1 = PopulateDayOfWeekNames();
                Dictionary<string, int> temp2 = PopulateMonthNames();
                Dictionary<string, int> temp3 = PopulateTimeZones();
            ...
            // Create a date from the components.
            DateTime result = new DateTime(year, month, 1, hour, minute, second, millisecond, kind);
        ...
        private static HashSet<string> PopulateDayOfWeekNames()
        {
            ...
            string[] abbreviatedNames = CultureInfo.InvariantCulture.DateTimeFormat.AbbreviatedDayNames;
            foreach (string dayName in abbreviatedNames) {
                result.Add(dayName);
            }

            // Add the full day names for the culture to the dictionary.
            string[] fullNames = CultureInfo.InvariantCulture.DateTimeFormat.DayNames;
        ...
        {
            Dictionary<string, int> monthNames = new Dictionary<string, int>(48, StringComparer.InvariantCultureIgnoreCase);

            // Add the abbreviated month names for the culture to the dictionary.
            string[] abbreviatedNames = CultureInfo.InvariantCulture.DateTimeFormat.AbbreviatedMonthNames;
            for (int i = 0; i < 12; i++) {
                monthNames[abbreviatedNames[i]] = i + 1;
            }

            // Add the full month names for the culture to the dictionary.
            string[] fullNames = CultureInfo.InvariantCulture.DateTimeFormat.MonthNames;
            for (int i = 0; i < 12; i++) {
        ...
        {
            Dictionary<string, int> result = new Dictionary<string, int>(15, StringComparer.InvariantCultureIgnoreCase)
            {
                { "UT", 0 },
                { "UTC", 0 },
                { "GMT", 0 },
