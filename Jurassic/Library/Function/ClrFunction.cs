...
namespace Jurassic.Library
{
    ...
    public class ClrFunction : FunctionInstance
    {
            ...
            : base(prototype)
        {
            ...
            List<JSBinderMethod> callBinderMethods = new List<JSBinderMethod>(1);
            List<JSBinderMethod> constructBinderMethods = new List<JSBinderMethod>(1);
            MethodInfo[] methods = this.GetType().GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly);
            foreach (MethodInfo method in methods)
            {
                // Search for the [JSCallFunction] and [JSConstructorFunction] attributes.
                JSCallFunctionAttribute callAttribute = (JSCallFunctionAttribute) Attribute.GetCustomAttribute(method, typeof(JSCallFunctionAttribute));
                JSConstructorFunctionAttribute constructorAttribute = (JSConstructorFunctionAttribute)Attribute.GetCustomAttribute(method, typeof(JSConstructorFunctionAttribute));
                ...
                {
                    JSBinderMethod binderMethod = new JSBinderMethod(method, constructorAttribute.Flags);
            ...
            this.callBinder = callBinderMethods.Count > 0
                ? new JSBinder(callBinderMethods)
                : new JSBinder(new JSBinderMethod(new Func<object>(() => Undefined.Value).Method));

            // Initialize the Construct function.
            this.constructBinder = constructBinderMethods.Count > 0
                ? new JSBinder(constructBinderMethods)
                : new JSBinder(new JSBinderMethod(new Func<ObjectInstance>(() => this.Engine.Object.Construct()).Method));
        ...
        public override object CallLateBound(object thisObject, params object[] arguments)
        {
            if (this.Engine.CompatibilityMode == CompatibilityMode.ECMAScript3)
            {
                ...
                thisObject = TypeUtilities.IsUndefined(thisObject) == true || thisObject == Null.Value
                    ? this.Engine.Global
                    : TypeConverter.ToObject(this.Engine, thisObject);
            }
            try
            {
                return this.callBinder.Call(this.Engine, this.thisBinding != null ? this.thisBinding : thisObject, arguments);
            }
