    ...
    public class ArgumentsInstance : ObjectInstance
    {
            ...
            : base(prototype)
        {
            ...
            if (this.callee.StrictMode == false)
            {
                ...
                Dictionary<string, int> mappedNames = new Dictionary<string, int>();    // maps argument name -> index
                for (int i = 0; i < argumentValues.Length; i++)
                {
                    if (i < callee.ArgumentNames.Count)
                    {
                        ...
                        UserDefinedFunction getter = new UserDefinedFunction(this.Engine.Function.InstancePrototype, "ArgumentGetter", new string[0], this.scope, "return " + callee.ArgumentNames[i], this.ArgumentGetter, true);
                        getter.SetPropertyValue("argumentIndex", i, false);
                        UserDefinedFunction setter = new UserDefinedFunction(this.Engine.Function.InstancePrototype, "ArgumentSetter", new string[] { "value" }, this.scope, callee.ArgumentNames[i] + " = value", this.ArgumentSetter, true);
            ...
            else
            {
                ...
                ThrowTypeErrorFunction throwErrorFunction = new ThrowTypeErrorFunction(this.Engine.Function.InstancePrototype);
        ...
        public override bool Delete(uint index, bool throwOnError)
        {
            ...
            if (index < this.mappedArguments.Length && this.mappedArguments[index] == true)
            {
                ...
                object currentValue = this.scope.GetValue(this.callee.ArgumentNames[(int)index]);
                this.DefineProperty(index.ToString(), new PropertyDescriptor(currentValue, PropertyAttributes.FullAccess), false);
            }
