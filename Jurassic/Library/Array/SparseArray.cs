    ...
    internal sealed class SparseArray
    {
        ...
        public object this[uint index]
        {
            get
            {
                ...
                return this.GetValueNonCached(index);
            }
            set
            {
                ...
                {
                    object[] array = this.FindOrCreateArray(index, writeAccess: true);
        ...
        public void Delete(uint index)
        {
            ...
            {
                object[] array = this.FindOrCreateArray(index, writeAccess: false);
                if (array == null) {
                    return;
                }
        ...
        public void DeleteRange(uint start, uint length)
        {
            ...
            this.DeleteRange(start, length, null, this.root, 0, this.depth);
        }
        ...
        private void DeleteRange(uint start, uint length, Node parentNode, Node node, int nodeIndex, int nodeDepth)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < NodeSize; i++)
                {
                    Node element = node.array[i] as Node;
                    if (element != null)
                    {
                        this.DeleteRange(start, length, node, element, i, nodeDepth - 1);
                    }
                }
            }
        }
        ...
        private object[] FindOrCreateArray(uint index, bool writeAccess)
        {
            ...
            for (int depth = this.depth - 1; depth > 0; depth--)
            {
                ...
                Node newNode = (Node)current.array[currentIndex];
                if (newNode == null)
                {
                    if (writeAccess == false) {
                        return null;
        ...
        public void CopyTo(object[] source, uint start, int length)
        {
            ...
            do
            {
                ...
                object[] dest = this.FindOrCreateArray(start, writeAccess: true);
        ...
        {
            uint originalStart = start;
            foreach (Range sourceRange in source.Ranges)
            {
                int sourceOffset = 0;
