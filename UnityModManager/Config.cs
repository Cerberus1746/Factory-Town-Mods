    ...
    public partial class UnityModManager
    {
        public sealed class Param
        {
            ...
            public void Save()
            {
                try
                {
                    this.ModParams.Clear();
                    foreach (ModEntry mod in modEntries)
                    {
                        this.ModParams.Add(new Mod { Id = mod.Info.Id, Enabled = mod.Enabled });
                    }
            ...
            public static Param Load()
            {
                if (File.Exists(filepath))
                {
                    try
                    {
                        ...
                        {
                            XmlSerializer serializer = new XmlSerializer(typeof(Param));
                            Param result = serializer.Deserialize(stream) as Param;
            ...
            {
                foreach (Mod item in this.ModParams)
                {
                    ModEntry mod = FindMod(item.Id);
                    if (mod != null)
                    {
