    ...
    public class Injector
    {
        ...
        private static void _Run(bool doorstop)
        {
            ...
            if (!string.IsNullOrEmpty(UnityModManager.Config.StartingPoint))
            {
                ...
                {
                    if (TryGetEntryPoint(UnityModManager.Config.StartingPoint, out Type @class, out MethodInfo method, out string place))
                    {
                        usePrefix = (place == "before");
                        HarmonyInstance harmony = HarmonyInstance.Create(nameof(UnityModManager));
                        MethodInfo prefix = typeof(Injector).GetMethod(nameof(Prefix_Start), BindingFlags.Static | BindingFlags.NonPublic);
                        MethodInfo postfix = typeof(Injector).GetMethod(nameof(Postfix_Start), BindingFlags.Static | BindingFlags.NonPublic);
            ...
            {
                if (TryGetEntryPoint(UnityModManager.Config.UIStartingPoint, out Type @class, out MethodInfo method, out string place))
                {
                    usePrefix = (place == "before");
                    HarmonyInstance harmony = HarmonyInstance.Create(nameof(UnityModManager));
                    MethodInfo prefix = typeof(Injector).GetMethod(nameof(Prefix_Show), BindingFlags.Static | BindingFlags.NonPublic);
                    MethodInfo postfix = typeof(Injector).GetMethod(nameof(Postfix_Show), BindingFlags.Static | BindingFlags.NonPublic);
                    harmony.Patch(method, new HarmonyMethod(prefix), new HarmonyMethod(postfix));
                }
                else
                {
        ...
        internal static bool TryGetEntryPoint(Assembly assembly, string str, out Type foundClass, out MethodInfo foundMethod, out string insertionPlace)
        {
            ...
            if (!TryParseEntryPoint(str, out _, out string className, out string methodName, out insertionPlace))
            {
                return false;
            }
